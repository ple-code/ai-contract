...
- **登录后默认进入合同列表（首页）**，而非直接上传。列表展示已上传合同：合同名称、编号、**合同类型**、**状态**（待审/审阅中/已完成）、当前版本、最近更新时间、当前处理人/负责人。
...
- 列表页提供**「上传新合同」入口**（走 F17 识别流程）；上传成功后该合同**出现在列表**并可进入工作台。
...
- Given 合同列表，When 点击"上传新合同"并成功上传采购合同，Then 新合同出现在列表中。
...
- **F8 审查标准统一（P1）**：不同法务侧重不同，期望初审提供统一标准，保证合同先到"60 分基础线"，个人用户能力再加分到 80 分。实现路径：先建立规则集，AI 基于规则评判是否符合。
...
- **F14 按岗位/关注点快速定位条款（P1）**：法务原话——不同**岗位**各看各关心的部分（如财务看结算/财务条款、法务确认关键信息、商务填商务信息），"根据使用者的角色来去定位到那个段落，就不用再去看整篇合同"，现状只能"翻翻翻"逐页找。这里的"角色"指**岗位**（F16，非甲乙中立立场）。期望：进入合同后按**登录岗位**自动定位/高亮该岗位关注的条款段落，并提供快速跳转导航，免翻整篇。与 F16（岗位）、F2（风险点定位）协同。MVP 可先做"法务=关键/风险条款、财务=付款/违约金、销售=交货、商务=商务信息"的岗位→关注条款映射 + 一键定位，后续细化。
...
- AI 结果定位为初审，须有法务复审环节，界面应体现"建议/待复审"语义而非"定论"。
